...
/**
 * EndLessons решение задачи: В некоторой школе занятия начинаются в 9:00. Продолжительность урока —
 * 45 минут, после 1-го, 3-го, 5-го и т.д. уроков перемена 5 минут, а после 2-го, 4-го, 6-го и т.д.
 * — 15 минут. Определите, когда заканчивается указанный урок. Данные вводятся с консоли Входные
 * данные: Дан номер урока (число от 1 до 10). Выходные данные: Выведите два целых числа: время
 * окончания урока в часах и минутах. При решении этой задачи нельзя пользоваться циклами и
 * условными инструкциями!!!!!!!!!!!!!!!!! Примеры: Входные данные: 3 Выходные данные:11 35 Входные
 * данные: 2 Выходные данные: 10 35
 */
public class EndLessons {
  ...
  public String MathematicalSolution(int i) {
    String result;
    long timeEndLesson = startLessons + ((TIME_ONE_LESSON) * i) + ((SHORT_PAUSE) * (i - 1)) + (
        ((LONG_PAUSE - SHORT_PAUSE)) * ((i - 1) / 2));
    System.out.println(
        myColors.ANSI_WHITE_BACKGROUND + myColors.ANSI_BLACK + "MathematicalSolution:"
            + myColors.ANSI_RESET);
  ...
  public String StupidDecision(int i) {
    ...
    System.out.println(myColors.ANSI_WHITE_BACKGROUND + myColors.ANSI_BLACK + "StupidDecision:"
        + myColors.ANSI_RESET);
